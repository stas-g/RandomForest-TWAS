...
FILE <- sprintf("model_output/chrm%s/%s_gwas_mods/ichip", k, a)

# #create progress indicators
# ind <- rep(FALSE, 22)
# for(a in c('knight', 'raj')){
#     file_ <- sprintf('ind-mtl-%s-ichip.rds', a)
#     message(file_)
...
# mm <- readRDS(sprintf('ind-mtl-%s-ichip.rds', a))
...
saveRDS(res, file = sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_ichip_rsq.rds", k, a))
...
mm <- readRDS(sprintf('ind-mtl-%s-ichip.rds', a))
mm[k] <- TRUE
saveRDS(mm, file = sprintf('ind-mtl-%s-ichip.rds', a))
...
#create progress indicators
for(a in c('knight', 'raj')){
    file_ <- readRDS(sprintf('/scratch/ng414/twas/ind-mtl-%s-ichip.rds', a))
    print(file_)
}

for(a in c('knight', 'raj')){
  for(k in 1 : 22){
    val <- file.exists(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_ichip_rsq.rds", k, a))
...
lapply(c('knight', 'raj'), FUN = function(a) {
  ...
  file.exists(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_ichip_rsq.rds", k, a))
    })
       ind <- which(!out)
       if(length(ind) > 1){
       ...
       message(sprintf('export dat=%s k=%s; sbatch gwas_mtl_ichip_mods-14112017.sh', aa, ind))
       message('')
     }
})
...
for a in {1,2}
  do export dat=$a
    for k in {1..22};
      do
        export chr=$k
          sbatch gwas_mtl_ichip_mods-14112017.sh
  done
done
...
zz <- lapply(c('knight', 'raj'), FUN = function(a) {
  lapply(1 : 22, FUN = function(k) {
    message(a, ' chrm ', k)
    x <- readRDS(sprintf("/scratch/wallace/twas/model_output/chrm%s/gwas_mtl_%s_ichip_rsq.rds", k, a))
    colnames(x)
  })
})
...
rsq <- lapply(c('knight', 'raj'), FUN = function(a) {
  lapply(1 : 22, FUN = function(k) {
    val <- readRDS(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_ichip_rsq.rds", k, a))

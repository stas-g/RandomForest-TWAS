...
FILE <- sprintf("model_output/chrm%s/%s_gwas_mods/%s", k, a, d)


# ind <- rep(FALSE, 22)
# for(a in c('knight', 'raj')){
#   for(d in c('t1dgc+wtccc', 'jia', 'ms')){
#     file_ <- sprintf('ind-mtl-%s-%s.rds', a, d)
#     message(file_)
...
# mm <- readRDS(sprintf('ind-mtl-%s-%s.rds', a, d))
...
run.mtl <- function(j, tr, a){
  mod.f <- file.path(FILE, sprintf("%s-%s.rds", j, paste(tr, collapse = '-')))
  ...
  rsq <- mod$rsq[500]
  rsq <- rep(rsq, length(tr))
  names(rsq) <- paste(dd, tr, j, sep = ".")
  rsq
}
...
run.stl <- function(j, tr, a){
  ...
  mod.f <- file.path(FILE, sprintf("%s-%s.rds", j, tr))
...
message('length of groups.fin is: %s', length(groups.fin))
print(FILE)
print(getwd)
...
}, mc.cores = 20) %>% do.call(c, .)


saveRDS(res, file = sprintf("/rds/user/ng414/hpc-work/twas/model_output/chrm%s/gwas_mtl_%s_%s_rsq.rds", k, a, d))
...
mm <- readRDS(sprintf('ind-mtl-%s-%s.rds', a, d))
mm[k] <- TRUE
saveRDS(mm, file = sprintf('ind-mtl-%s-%s.rds', a, d))


q('no')
#----------------------------
resr <- lapply(1 : 22, FUN = function(k) {
  x <- readRDS(sprintf("model_output/chrm%s/gwas_mtl_%s_%s_rsq.rds", k, a, d))
  x
    })
...
for(a in c('knight', 'raj')){
  ...
  for d in c('t1dgc+wtccc', 'jia', 'ms'){
      res <- readRDS(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_%s_rsq.rds", k, a, d))
      res <- res[, !(colnames(res) %in% hla)]
      ###!!! need to extract probe names from colnames, then subset by hla (28.09.17)
      saveRDS(res, sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_%s_rsq.rds", k, a, d))
  }
}
...
for dd in {1,2,3}
  do export d=$dd
    for k in {1..22};
      do
        export chr=$k
          sbatch gwas_mtl_mods-14112017.sh
  done
done
...
for(a in c('knight', 'raj')){
  for(d in c('t1dgc+wtccc', 'jia', 'ms')){
    file_ <- readRDS(sprintf('/scratch/ng414/twas/ind-mtl-%s-%s.rds', a, d))
    message(a, ' ', d)
    print(file_)
...
for(a in c('knight', 'raj')){
  for(d in c('t1dgc+wtccc', 'jia', 'ms')){
    for(k in 1 : 22){
      val <- file.exists(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_%s_rsq.rds", k, a, d))
...
lapply(c('knight', 'raj'), FUN = function(a) {
  ...
  lapply(c('t1dgc+wtccc', 'jia', 'ms'), FUN = function(d) {
   ...
   file.exists(sprintf("/rds/user/ng414/hpc-work/twas/model_output/chrm%s/gwas_mtl_%s_%s_rsq.rds", k, a, d))
    })
       ind <- which(!out)
       if(length(ind) > 1){
         ...
         message('      sbatch gwas_mtl_mods-14112017.sh')
         message('done')
         message('')
     } else {
       message(sprintf('export dat=%s d=%s chr=%s; sbatch gwas_mtl_mods-14112017.sh', aa, dd, ind))
       message('')
     }
  })
})
...
zz2 <- lapply(1 : length(groups.fin), FUN = function(i){
  ...
  rsq <- lapply(c('knight', 'raj'), FUN = function(a) {
    lapply(c('t1dgc+wtccc', 'jia', 'ms'), FUN = function(d) {
      lapply(1 : 22, FUN = function(k) {
        val <- readRDS(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_mtl_%s_%s_rsq.rds", k, a, d))
        out <- strsplit(names(val), split = '\\.') %>% do.call(rbind, .)
        data.frame(out, val)
    }) %>% do.call(rbind, .)
        }) %>% do.call(rbind, .)
  }) %>% do.call(rbind, .)
  colnames(rsq) <- c('cell', 'probe', 'rsq')
  rsq$cell <- gsub('cd', 'CD', rsq$cell)

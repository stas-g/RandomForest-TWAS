...
FILE.M <- sprintf("model_output/chrm%s/%s_gwas_mtl_mods/ichip", k, a)
FILE.P <- sprintf("model_output/chrm%s/%s_gwas_mtl_preds/ichip", k, a)
...
saveRDS(res, file = sprintf("model_output/chrm%s/gwas_mtl_%s_ichip_res.rds", k, a))
...
rsq.mtl <- mclapply(1 : 22 , FUN = function(k) {
    out <- lapply(c('knight', 'raj'), FUN = function(a) {
        path <- sprintf("model_output/chrm%s/gwas_mtl_%s_mods/ichip", k, a)

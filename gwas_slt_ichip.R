...
FILE <- sprintf("model_output/chrm%s/%s_ichip_stl_rf_preds", k, a)

# #create progress indicators
# ind <- rep(FALSE, 22)
# for(a in c('knight', 'raj')){
#     file_ <- sprintf('ind-%s-slt-rf-ichip_preds.rds', a)
#     message(file_)
...
# mm <- readRDS(sprintf('ind-%s-slt-rf-ichip_preds.rds', a))
...
run.stl <- function(j){
  ...
  if(status){
    ...
    message(sprintf("STL: chrm %s cell %s: %s ok!", k, p, j))
  }

  message(sprintf("STL: chrm %s cell %s: %s done!", k, p, j))
  pred
}
...
for(i in 1 : length(pheno.names)) {
  ...
  res <- mclapply(iind, FUN = function(j){
          ...
          message(j)
          run.stl(j)
              }, mc.cores = 7, mc.preschedule = FALSE) %>% do.call(cbind, .)
...
saveRDS(pred, file = sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_stl_rf_%s_ichip_res.rds", k, a))
...
mm <- readRDS(sprintf('ind-%s-slt-rf-ichip_preds.rds', a))
mm[k] <- TRUE
saveRDS(mm, file = sprintf('ind-%s-slt-rf-ichip_preds.rds', a))
...
#create progress indicators
for(a in c('knight', 'raj')){
    file_ <- readRDS(sprintf('/scratch/wallace/twas/ind-%s-slt-rf-ichip.rds', a))
    print(file_)
}

for(a in c('knight', 'raj')){
  for(k in 1 : 22){
    val <- file.exists(sprintf("/scratch/wallace/twas/model_output/chrm%s/gwas_stl_rf_%s_ichip_res.rds", k, a))
...
lapply(c('knight', 'raj'), FUN = function(a) {
  ...
  file.exists(sprintf("/scratch/wallace/twas/model_output/chrm%s/gwas_lasso_%s_ichip_res.rds", k, a))
    })
       ind <- which(!out)
       if(length(ind) > 1){
       ...
       message(sprintf('export dat=%s k=%s; sbatch gwas_ichip_lasso-07082017.sh', aa, ind))
       message('')
     }
})
...
for a in {1,2}
  do export dat=$a
    for k in {1..22};
      do
        export chr=$k
          sbatch gwas_slt_ichip-30082017.sh
  done
done

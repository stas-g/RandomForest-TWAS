...
# FILE <- sprintf("model_output/chrm%s/%s_gwas_lasso_preds/ichip", k, a)
FILE <- sprintf("model_output/chrm%s/%s_gwas_lasso_mods/ichip", k, a)

# #create progress indicators
# ind <- rep(FALSE, 22)
# for(a in c('knight', 'raj')){
#     file_ <- sprintf('ind-%s-lasso-ichip.rds', a)
#     message(file_)
...
# mm <- readRDS(sprintf('ind-%s-lasso-ichip.rds', a))
...
for(i in 1 : length(pheno.names)) {
  ...
  res <- mclapply(iind, FUN = function(j){
          ...
          message(j)
          run.stl(j)
              }, mc.cores = 5) %>% do.call(c, .)

              rsq[[i]] <- res
}

saveRDS(rsq, file = sprintf("model_output/chrm%s/gwas_lasso_%s_ichip_rsq.rds", k, a))
...
mm <-  readRDS(sprintf('ind-%s-lasso-ichip.rds', a))
mm[k] <- TRUE
saveRDS(mm, file = sprintf('ind-%s-lasso-ichip.rds', a))
...
#create progress indicators
for(a in c('knight', 'raj')){
    file_ <- readRDS(sprintf('/scratch/wallace/twas/ind-%s-lasso-ichip.rds', a))
    print(file_)
}

for(a in c('knight', 'raj')){
  for(k in 1 : 22){
    val <- file.exists(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_lasso_%s_ichip_rsq.rds", k, a))
...
lapply(c('knight', 'raj'), FUN = function(a) {
  ...
  file.exists(sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_lasso_%s_ichip_rsq.rds", k, a))
    })
       ind <- which(!out)
       if(length(ind) > 1){
       ...
       message(sprintf('export dat=%s k=%s; sbatch gwas_lasso_ichip_mods-09112017.sh', aa, ind))
       message('')
     }
})
...
for a in {1,2}
  do export dat=$a
    for k in {1..22};
      do
        export chr=$k
          sbatch gwas_lasso_ichip_mods-09112017.sh
  done
done

...
FILE <- sprintf("model_output/chrm%s/%s_gwas_stl_rf_mods/%s", k, a, d)

# k <- 6
# a <- 'raj'
# for(d in c('t1dgc+wtccc', 'jia')){
#   message(sprintf("rm /scratch/ng414/twas/model_output/chrm%s/%s_gwas_stl_rf_mods/%s/*", k, a, d))
# }
...
#     file_ <- sprintf('ind-stl-rf-%s-%s.rds', a, d)
#     message(file_)
...
# mm <- readRDS(sprintf('ind-stl-rf-%s-%s.rds', a, d))
...
saveRDS(rsq, file = sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_stl_rf_%s_%s_rsq.rds", k, a, d))
...
mm <- readRDS(sprintf('ind-stl-rf-%s-%s.rds', a, d))
mm[k] <- TRUE
saveRDS(mm, file = sprintf('ind-stl-rf-%s-%s.rds', a, d))
...
for(a in c('knight', 'raj')){
  for(d in c('t1dgc+wtccc', 'jia', 'ms')){
    file_ <- readRDS(sprintf('/scratch/ng414/twas/ind-stl-rf-%s-%s.rds', a, d))
    message(a, ' ', d)
    print(file_)
  }
}
...
lapply(c('knight', 'raj'), FUN = function(a) {
  ...
  lapply(c('t1dgc+wtccc', 'jia', 'ms'), FUN = function(d) {
   ...
   file.exists(sprintf("/rds/user/ng414/hpc-work/twas/model_output/chrm%s/gwas_stl_rf_%s_%s_rsq.rds", k, a, d))
    })
       ind <- which(!out)
       if(length(ind) > 1){
       ...
       message(sprintf('export dat=%s d=%s chr=%s; sbatch gwas_stl_mods-08112017.sh', aa, dd, ind))
       message('')
     }
  })
})
...
for dd in {1,2,3}
  do export d=$dd
    for k in {1..22};
      do
        export chr=$k
          sbatch gwas_stl_mods-08112017.sh
  done
done

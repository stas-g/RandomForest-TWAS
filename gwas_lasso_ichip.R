...
#     file_ <- sprintf('ind-%s-lasso-ichip_preds.rds', a)
#     message(file_)
...
# mm <- readRDS(sprintf('ind-%s-lasso-ichip_preds.rds', a))
...
saveRDS(pred, file = sprintf("/scratch/ng414/twas/model_output/chrm%s/gwas_lasso_%s_ichip_res.rds", k, a))
...
mm <-  readRDS(sprintf('ind-%s-lasso-ichip_preds.rds', a))
mm[k] <- TRUE
saveRDS(mm, file = sprintf('ind-%s-lasso-ichip_preds.rds', a))
...
for(a in c('knight', 'raj')){
  for(k in 1 : 22){
    val <- file.exists(sprintf("/scratch/wallace/twas/model_output/chrm%s/gwas_lasso_%s_ichip_res.rds", k, a))
...
for a in {1,2}
  do export dat=$a
    for k in {1..22};
      do
        export chr=$k
          sbatch gwas_ichip_lasso-07082017.sh
  done
done
